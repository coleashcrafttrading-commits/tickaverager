...
RANK 1 of the strategy study. 15Min bars, 250 days, 50 symbols, out of sample.

    total P/L            +80,349        100 shares a trade, summed over 50 symbols
    return on capital    4.77%
    vs buy and hold      +85,117   (beat it on 28 of 50 symbols)
    vs passive twin      +79,680   (beat it on 39 of 50 symbols)
    trades               10,252
    median win rate      52.9%
    pooled profit factor 1.174
    worst drawdown       -18,329   on a single symbol
    median exposure      9.8%   of all bars
...
Source: StockSharp/AlgoTrading, strategy API/1601-1700/1601_Consecutive_Close_High1_Mean_Reversion -- 'counts consecutive closes above the prior high and sells once the count reaches a threshold; position exits when price falls 
"""
...
def family_init(ctx):
    ...
    run = 0
    for j in range(1, n):
        run = run + 1 if closes[j] > highs[j - 1] else 0
        k[j] = run
    ctx.k = ctx.series(k, "streak_count")
