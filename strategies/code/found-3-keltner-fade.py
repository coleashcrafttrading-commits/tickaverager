"""
Keltner fade  --  found by the research pass on 2026-09-02

Same shape as the Bollinger fade but on an ATR channel, so it does not widen just because the last twenty closes disagreed.

WHAT THE BACKTEST SAID (out of sample, on data never used to choose anything)
    score (P/L per $ drawdown, median symbol)   +0.266
    in-sample score, for comparison             +1.158
    decay out of sample                         0.23
    profitable on                               5 of 8 symbols
    trades                                      724
    summed P/L at 100 shares                    +$3,689.82
    control (random entry) had to be beaten at  -0.022
    beats that control                          yes
    at 2x the assumed slippage             +0.049
