...
# Call sites already logged through LOG and this module never defined it, so
# every one of them was a NameError waiting for its branch -- in the handler
# whose whole job was to degrade quietly.
...
# ===================================================================== optlab
# THE RESEARCH AND EVIDENCE SIDE OF OPTIONS, and nothing else: a bank of
# strategy documents, a chain with greeks computed here, and the saved
# backtest sweeps with their grades. Every route is a GET and none of them can
# place an order.
#
# The namespace is /api/optlab/... on purpose. optapi.py -- Glenn's engine,
# mounted as a router on this same app -- owns /api/options/*, and two files
# registering /api/options/chain/{sym} do not collide loudly: FastAPI matches
# the first one registered and the other simply never runs, which is how two
# chains with different pricing quietly became one. test_options_api.py pins
# that the two namespaces stay disjoint.
#
# WHICH TRADES: optexec.py, through optapi.py's stack, is the only path that
# may send an option order, and live positions are on its own /options page.
# This section had a second write path once -- proposal/submit/close driven
# off an OptionEngine class that no longer exists -- and it was removed rather
# than left dead, because a dead order path is one somebody wires back up.
#
...
# owns the maths, and optdata.py owns the fetching. These routes are the
