...
class Engine:
            ...
            if status == "filled":
                ...
                if lot in self.ledger.open_lots and lot.shares > QTY_EPS:
                    ...
                    self.ev("INFO" if (status == "expired" and not qwhole(lot.shares)) else "WARN",
                            f"TP for lot {lot.id} is {status} -- re-placing "
                            f"for its remaining {qstr(lot.shares)} sh.")
                    lot.tp_client_id = ""
                    lot.tp_order_id = ""
    ...
    def _close_lot_now(self, lot: "Lot", why: str) -> bool:
        ...
        frac = not qwhole(lot.shares)
        if frac and self._frac_wait(lot.id):
            return False                     # BEFORE the cancel: a refused exit must not leave the lot naked
        try:
            if lot.tp_client_id:
                o = next((x for x in self.open_orders
                          if x.get("client_order_id") == lot.tp_client_id), None)
            ...
            if self.is_extended():
                ...
                # (a fractional lot goes DAY; a whole one GTC as before)
                if short:
                    ref = float(self.quote.get("ap") or 0) or self.last_price
                    px = _round_cent(max(0.01, ref + 0.02))
                    o = self._limit_place(b, "buy", lot.shares)(self.symbol, qnum(lot.shares), px, coid,
                                                                extended_hours=True)
                else:
                    ref = float(self.quote.get("bp") or 0) or self.last_price
                    px = _round_cent(max(0.01, ref - 0.02))
                    o = self._limit_place(b, "sell", lot.shares)(self.symbol, qnum(lot.shares), px, coid,
                                                                 extended_hours=True)
            else:
                o = b.submit(symbol=self.symbol, qty=qnum(lot.shares),
    ...
    def _lot_shares(self, price: float = 0.0, extra_deployed: float = 0.0):
        ...
        if frac and n * price < MIN_NOTIONAL:
            ...
            return 0
        return qnum(self._cap_to_ladder(n, price, extra_deployed))
    ...
    def _book_resting_add(self, rec: dict, o: dict) -> None:
        ...
        if existing is not None:
            ...
            if existing.shares + QTY_EPS >= filled:
                rec["booked"] = filled
                self.ledger.save()
                return
